...
html='''{% extends 'base.html' %}
    ...
    '''.format(html1=html1,html2=html2)+'''
    {% endblock %}
    '''

with open(dir_path+'/templates/sea_all.html','w')as f:
    f.write(html)
       
app = Flask(__name__)

@app.route('/',methods=["GET"])  #生成泰国参观数和景观数
def all_sea():
    
    
    return render_template('sea_all.html',
                           tables=[qp.to_html(classes='data')],
                           titles=qp.columns.values,
                           )
...
def map():
    ...
    '''.format(div1=div1,div2=div2,html1=html1,html2=html2,html3=html3)+'''
    {% endblock %}
    '''
    with open(dir_path+'/templates/try.html','w') as f:
        f.write(html)
